...
PROGRAM W2_PRE
  ...
  USE DFLIB
  USE CONTROL; USE GRIDC; USE KINETICS; USE PREC; USE FILE_ERROR; use macrophytec; use zooplanktonc; USE HABITAT
...
INTEGER       :: nbodcs, nbodce, nbodps, nbodpe, nbodns, nbodne, ibod, jcb
  ...
  INTEGER(4)    :: length,istatus,tcnelev,tciseg,tspltjb,numtsplt,tempn,jstsplt(100)
...
! Allocation declarations
  ...
  REAL,          ALLOCATABLE, DIMENSION(:)     :: AX,     AZMAX,  T2I,    ELBOT,  SLOPE,  LAT,    LONG, SLOPEC
  ...
  REAL,          ALLOCATABLE, DIMENSION(:,:,:) :: CVP,    EPIVP, EPD
...
length = GETDRIVEDIRQQ(moddir)
  ...
  STAT = ABOUTBOXQQ ('CE-QUAL-W2 Preprocessor V3.7 - Waterways Experiment Station and PSU Dept. Civil&Environmental Engineering'C)   ! SW 1/16/04
  WRITE (WINTITLE,'(A29,A300)') 'CE-QUAL-W2 3.7 preprocessor. ',moddir
...
IF(SELECTC /= '     OFF' .AND. SELECTC /= '      ON')THEN
 CALL ERRORS
 WRITE(ERR,*)'SELECTC MUST BE "OFF" OR "ON". SELECTC=',SELECTC
ENDIF

IF(SELECTC == '      ON')THEN      ! CHECK FILE SELECTC
unit=1010
      ...
      read(1010,'(a8,a8,i8,i8,a8,f8.0,f8.0,f8.0,i8,10(f8.0))',err=400)aid1,tcntr(j),tcjb(j),tcjs(j),tcyearly,tctsrt,tctend,tctemp,tcnelev,(tcelev(n),n=1,tcnelev)
       ...
       if(tctsrt < tmstrt  .and. tcyearly /= '      ON')then
        ...
        if(tctsrt > tmend)then
         ...
         call errors
         write(err,'(a,f10.3,a,f10.3,a,i3)')'w2_selective: DYNSTR2 TEND must be >= Time of model start [TMSTRT=',tmstrt,']. TEND=',tctsrt,' for #',j
       endif
        if(tctend > tmend)then
         call warnings   ! changed to a warning rather than an error
         write(wrn,'(a,f10.3,a,f10.3,a,i3)')'w2_selective: DYNSTR2 TEND must be <= model end time [TMEND=',tmend,']. TEND=',tctsrt,' for #',j
       endif
 ...
 !         if(NSTR(tcjb(j))==0)then
      ...
      if(tctsrt < tmstrt  .and. tcyearly /= '      ON')then
        ...
        if(tctsrt > tmend)then
         ...
         if(tctend < tmstrt  .and. tcyearly /= '      ON')then
         call errors
         write(err,'(a,f10.3,a,f10.3,a,i3)')'w2_selective: SPLIT TEMP TEND must be >= Time of model start [TMSTRT=',tmstrt,']. TEND=',tctsrt,' for #',j
       endif
        if(tctend > tmend)then
         call errors
         write(err,'(a,f10.3,a,f10.3,a,i3)')'w2_selective: SPLIT TEMP TEND must be <= model end time [TMEND=',tmend,']. TEND=',tctsrt,' for #',j
        endif
...
unit=5000
   OPEN (5000,FILE='w2_envirpf.npt',STATUS='UNKNOWN',IOSTAT=IERR)
  IF (IERR /= 0) THEN
    WRITE (WIN,*) 'Could not open w2_envirpf.npt input file'
    CALL ERRORS
    WRITE(ERR,*)'Could not open w2_envirpf.npt input file'
  ELSE
    ! check file
  END IF
  CLOSE(5000)
        ...
        IF (LONG_EPIPHYTON(JW,JE).AND.ICHAR1=='$')READ (LPR(JW),*,ERR=400)
           ...
           DO I=CUS(JB),DS(JB)
              IF (LONG_EPIPHYTON(JW,JE))THEN
                  IF(ICHAR1=='$')THEN
                    READ (LPR(JW),*,ERR=400) AID,(EPD(K,I,JE),K=KT,KB(I))
                      ELSE
                    READ (LPR(JW),'(/A8/(8X,9F8.0))',IOSTAT=IERR,ERR=400) AID,(EPD(K,I,JE),K=KT,KB(I))
                ENDIF
              ENDIF
...
! Initial input and conditions
  ...
  WRITE (INI,'(1X,A/)') 'CE-QUAL-W2 preprocessor - V3.7'
