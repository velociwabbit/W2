...
! open ancillary control file for fish bioenergetics !mlm bioexp
  ...
  OPEN (1222,FILE='W2_con_anc.npt',status='old', IOSTAT=I)
  IF (I /= 0) THEN
    FISHBIO = .FALSE.
  ELSE
    FISHBIO = .TRUE.
  ...
  IF(FISHBIO)THEN
   DO II = 1,16
    READ(1222,'(A8)') BIOC ! DUMMY VARIABLE AT THIS POINT FIX THIS
   END DO
   ENDIF
...
!        deALLOCATE (ESTR,WSTR,QSTR,KTSW,KBSW,SINKC,POINT_SINK,QNEW) 
!        deALLOCATE (ESTR,WSTR,QSTR,KTSW,KBSW,SINKC,POINT_SINK,QNEW,tavg, tavgw)
        b=bsave
        call initgeom
        call initial_u_velocity       
        open(999,file='init_wl_u_check.dat',status='unknown')
        write(999,'("       i elws_calc    qssi       u   depth elws_init")')
        DO JW=1,NWB        
          DO JB=BS(JW),BE(JW)
            IU = CUS(JB)
            ID = DS(JB)
            do i=iu,id
              ...
              write(999,'(i8,f8.3,f8.2,f8.3,2f8.2)')i,elws(i),qssi(i),u(kt,i),depth,elwss(i)
            end do
          end do
        end do
        close(999)
