  ...
  ALLOCATE(DZMULT(KMX,IMX))
   DZMULT=1.0
   OPEN(9540,FILE='W2_AERATE.NPT',STATUS='OLD')
   READ (9540,'(//I8,A16)')NAER,CONAER
...
1013 FORMAT(/)
      DO I=1,NAER
        READ(9540,'(I8,I8,I8,F8.0,F8.0,F8.0,3F8.0,2I8)')IASEG(I),KTOPA(I),KBOTA(I),SMASS(I),ATIMON(I),ATIMOFF(I),DZMULTA(I),DOOFF(I),DOON(I),IPRB(I),KPRB(I)
      END DO

	CLOSE(9540)

IF(RESTART_IN)THEN
  OPEN(9540,FILE=CONAER,POSITION='APPEND')
        JDAY1=0.0
        REWIND (9540)
        READ   (9540,'(//)')
        DO WHILE (JDAY1 < JDAY)
          READ (9540,'(F9.0)',END=106) JDAY1
        END DO
        BACKSPACE (9540)
        106     JDAY1=0.0
ELSE
  OPEN(9540,FILE=CONAER,STATUS='UNKNOWN')
  WRITE(9540,'(A,I3,A)')'OUTPUT FILE FOR AERATION INPUT WITH',NAER,' INPUT(S).'
  WRITE(9540,'(A114)')'JDAY,INSTMASSRATE#1(KGO2/D),CUMMASS#1(KGO2),DOPROBE#1(MG/L),INSTMASSRATE#2(KGO2/D),CUMMASS#2(KGO2),DOPROBE#2(MG/L)'
ENDIF
...
ENTRY AERATEOUTPUT     ! FROM W2_MAIN CODE
        ...
        WRITE(9540,'(F9.3,<NAER>(1X,E12.3,1X,E12.3,1X,F8.3))')JDAY,(ACTUAL_MASS(II),CUMDOMASS(II),O2(KPRB(II),IPRB(II)),II=1,NAER)
